...
<<<<<<< HEAD
*Starvation will occur in the Concurrent Programming if the pthread is not uesd correctly. A thread with too low a priority, or a thread that holds a lock without releasing it while looping indefinitely, or a program that keeps holding a write lock on a file can cause Starvation. If we 'lock' the shared resource for some 'painters' impertinently, those 'painters' will not go though as expected becasuse there exits other processes are going all the time. That is the starvation, which is the concurrency caveat we should takes some extra care. we may choose 'lock' and 'trylock' carefully so let as many as painters go into and paint equally without improper priority.*
