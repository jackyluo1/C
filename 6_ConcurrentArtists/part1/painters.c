...
// Write out the PPM file
	...
	for(int x =0; x < CANVAS_WIDTH; ++x){
		for(int y =0; y < CANVAS_HEIGHT; ++y){
			...
			fputs(" ",fp);	
                //      pthread_mutex_unlock(&canvas[x][y].lock);
		}
		fputs("\n",fp);
	}
	fclose(fp);	
...
void* paint(void* args){
	...
	for(int i =0; i < 5000; ++i){
       ...
       if(pthread_mutex_trylock(&canvas[painter->x][painter->y].lock)==0){
             sleep(5);
             pthread_mutex_unlock(&canvas[painter->x][painter->x].lock);
       }
...
int main(){
 ...
 //       pthread_mutex_init(&mutex,NULL);
        ...
        for(int i =0; i < rookieArtists; ++i){
            ...
            moreArtists[i].r = rand()%254;
            moreArtists[i].g = rand()%254;
            moreArtists[i].b = rand()%254;
            
            srand(time(NULL));

            pthread_create(&moreArtists_tid[i],NULL,(void*)paint,&i); 
