...
/*    int i = low,j = high;
    char* temp =array[i];
    while(i<j){
         while(i<j && strcmp(array[j],temp)>0){
               j--;
               array[i]=array[j];
         }
         while(i<j && strcmp(array[i],temp)<0){
               i++;
               array[j]=array[i];
         }
         array[j] = array[i];
    ...
    swapStrings(&array[i+1],&array[high-1]); 
...
int main(){
 ...
 /* 
  char** pivot = &musicDatabase1[13593];
   printf("%s",*pivot);
   printf("%s",musicDatabase1[13593]);
*/
