...
1. *A completely balanced binary tree has all nodes at depth d-1 with exactly 2 children.All nodes are ‘pushed’ to the left of a tree (no gaps)
The number of levels in a balanced binary tree is O(log2(n))*
2. *O(log2(n))*
3. *Yes.It can cause a very ‘unbalanced tree’ thus giving us O(n) search--even if we are using a binary search in our tree if all the nodes are in a line.*
4. *T(N) = 2T(N/2) + f(n).*
